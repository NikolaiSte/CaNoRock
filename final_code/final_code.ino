...
void irq_encoder(void){
    ...
    if(counter_frame%20 == 0){
        for(i = 1; i < 18; i++){
            to_encoder[1+counter_frame+i] = imu_buffer[i-1];
        }
    }

    if(counter_frame%40 == 0){
        for(i = 1; i < 18; i++){
            to_encoder[1+counter_frame+i] = imu2_buffer[i-1];
        }
    }

    if(counter_frame%60 == 0){
        for(i = 1; i < 18; i++){
            to_encoder[1+counter_frame+i] = imu3_buffer[i-1];
        }
    ...
    Serial.println(to_encoder[counter_frame]);
    Serial.print("INV-values:");
    Serial.print(imu3.accelInG.x,4);
    Serial.print(", ");
    Serial.print(imu3.accelInG.y,4);
    Serial.print(", ");
    Serial.println(imu3.accelInG.z,4);
    Serial.print("INV-values (raw): ");
    Serial.print(imu3.accelRaw.x);
    Serial.print(", ");
    Serial.print(imu3.accelRaw.y);
    Serial.print(", ");
    Serial.println(imu3.accelRaw.z);
    Serial.print("imu3_buffer 0 og 1: ");
    Serial.print(imu3_buffer[0]);
    Serial.print(", ");
    Serial.println(imu3_buffer[1]);
    Serial.println("");
    */
}
